...
static serial_triple_state_t serial_a1;  // UART1 — first A-side  (GPIO20/21/26/27)
static serial_triple_state_t serial_a2;  // UART0 — second A-side (GPIO12/13/14/15)

uint8_t buffer[SERIAL_TRIPLE_MAX_PAYLOAD_SIZE + sizeof(device_connected_t)];
...
void request_b_init() {
    ...
    serial_triple_write_nonblocking_channel(&serial_a1, (uint8_t*) &msg, sizeof(msg));
    serial_triple_write_nonblocking_channel(&serial_a2, (uint8_t*) &msg, sizeof(msg));
}

int main() {
    serial_triple_init_channel(&serial_a1, uart1, SERIAL_TX_PIN, SERIAL_RX_PIN, SERIAL_CTS_PIN, SERIAL_RTS_PIN);
    serial_triple_init_channel(&serial_a2, uart0, SERIAL2_TX_PIN, SERIAL2_RX_PIN, SERIAL2_CTS_PIN, SERIAL2_RTS_PIN);
    board_init();
    stdio_init_all();

    while (!initialized) {
        request_b_init();
        serial_triple_read_channel(&serial_a1, serial_callback);
        serial_triple_read_channel(&serial_a2, serial_callback);
    }
...
void report_received_callback(uint8_t dev_addr, uint8_t instance, uint8_t const* report, uint16_t len) {
    ...
    serial_triple_write_nonblocking_channel(&serial_a1, (uint8_t*) msg, len + sizeof(report_received_t));
    serial_triple_write_nonblocking_channel(&serial_a2, (uint8_t*) msg, len + sizeof(report_received_t));
}
...
void descriptor_received_callback(uint16_t vendor_id, uint16_t product_id, const uint8_t* report_descriptor, int len, uint16_t interface, uint8_t hub_port, uint8_t itf_num) {
    ...
    serial_triple_write_channel(&serial_a1, (uint8_t*) msg, len + sizeof(device_connected_t));
    serial_triple_write_channel(&serial_a2, (uint8_t*) msg, len + sizeof(device_connected_t));
}
...
void umount_callback(uint8_t dev_addr, uint8_t instance) {
    ...
    msg.dev_addr = dev_addr;
    msg.interface = instance;
    serial_triple_write_channel(&serial_a1, (uint8_t*) &msg, sizeof(msg));
    serial_triple_write_channel(&serial_a2, (uint8_t*) &msg, sizeof(msg));
}
...
void get_report_cb(uint8_t dev_addr, uint8_t interface, uint8_t report_id, uint8_t report_type, uint8_t* report, uint16_t len) {
    ...
    memcpy(msg->report, report, len);
    serial_triple_write_channel(&serial_a1, (uint8_t*) msg, len + sizeof(get_feature_response_t));
    serial_triple_write_channel(&serial_a2, (uint8_t*) msg, len + sizeof(get_feature_response_t));
}
...
void set_report_complete_cb(uint8_t dev_addr, uint8_t interface, uint8_t report_id) {
    ...
    msg->interface = interface;
    msg->report_id = report_id;
    serial_triple_write_channel(&serial_a1, (uint8_t*) msg, sizeof(set_feature_complete_t));
    serial_triple_write_channel(&serial_a2, (uint8_t*) msg, sizeof(set_feature_complete_t));
}
...
void tuh_midi_rx_cb(uint8_t dev_addr, uint32_t num_packets) {
    ...
    while (tuh_midi_packet_read(dev_addr, msg->msg)) {
        serial_triple_write_nonblocking_channel(&serial_a1, (uint8_t*) msg, sizeof(midi_received_t));
        serial_triple_write_nonblocking_channel(&serial_a2, (uint8_t*) msg, sizeof(midi_received_t));
    }
}
